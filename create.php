...
$time_slot_err = $mon_subject_err = $mon_time_err = $mon_teacher_err = $tue_subject_err = $tue_time_err = $tue_teacher_err = $wed_subject_err = $wed_time_err = $wed_teacher_err = $thur_subject_err = $thur_time_err = $thur_teacher_err = $fri_subject_err = $fri_time_err = $fri_teacher_err = $sat_subject_err = $sat_time_err = $sat_teacher_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($input_name)){
        $name_err = "<t class='text-danger'>*Please enter a name.</t>";
    } elseif(!filter_var($input_name, FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/")))){
        $name_err = "<t class='text-danger'>*Please enter a valid name.</t>";
    } else{
        $name = $input_name;
    }*/
    ...
    if(empty($time_slot_err) && empty($mon_subject_err) && empty($mon_time_err) && empty($mon_teacher_err) && empty($tue_subject_err) && empty($tue_time_err) && empty($tue_teacher_err) && empty($wed_subject_err) && empty($wed_time_err) && empty($wed_teacher_err) && empty($thur_subject_err) && empty($thur_time_err) && empty($thur_teacher_err) && empty($fri_subject_err) && empty($fri_time_err) && empty($fri_teacher_err) && empty($sat_subject_err) && empty($sat_time_err) && empty($sat_teacher_err)){
        ...
        $sql = "INSERT INTO grade1 (time_slot, mon_subject, mon_time, mon_teacher, tue_subject, tue_time, tue_teacher, wed_subject, wed_time, wed_teacher, thur_subject, thur_time, thur_teacher, fri_subject, fri_time, fri_teacher, sat_subject, sat_time, sat_teacher) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
 
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("sssssssssssssssssss", $param_time_slot, $param_mon_subject, $param_mon_time, $param_mon_teacher, $param_tue_subject, $param_tue_time, $param_tue_teacher, $param_wed_subject, $param_wed_time, $param_wed_teacher, $param_thur_subject, $param_thur_time, $param_thur_teacher, $param_fri_subject, $param_fri_time, $param_fri_teacher, $param_sat_subject, $param_sat_time, $param_sat_teacher);
                    ...
                    <div class="card mt-5">
                        <div class="card-body">
                            <div class="page-header">
                                <h2 class="display-4">Create Time Slot</h2>
                            </div>
                    <p>Please fill this form and submit to add record to the database.</p><br>
